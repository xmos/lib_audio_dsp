...
//#################################################################################################
	...
	static fft_constants_t constants3 = {.nfft = 512, .exp=(/**log2(512)=**/9 + SIG_EXP - 1)};
    ...
    // static bfp_multiply_config_t config4 = {};
    static bfp_multiply_state_t state4 = {};
	static bfp_multiply_constants_t constants4 = {.nfft = 512, .exp=(/**log2(512)=**/9 + SIG_EXP - 1)};
	static uint8_t memory4[BFP_MULTIPLY_STAGE_REQUIRED_MEMORY];
	static adsp_bump_allocator_t allocator4 = ADSP_BUMP_ALLOCATOR_INITIALISER(memory4);
	adsp_auto.modules[4].state = (void*)&state4;
	adsp_auto.modules[4].constants = (void*)&constants4;

	bfp_multiply_init(&adsp_auto.modules[4], &allocator4, 2, 1, 1, 256);


    // static ifft_config_t config5 = {};
    static ifft_state_t state5 = {};
	static ifft_constants_t constants5 = {.nfft = 512, .exp=(/**log2(512)=**/9 + SIG_EXP - 1)};
...
void dsp_auto_thread0(chanend_t* c_source, chanend_t* c_dest, module_instance_t** modules) {
	...
	SELECT_RES(
		...
		case_0: {
			chan_in_buf_word(c_source[0], (uint32_t*)edge0, 256);
			for(int idx = 0; idx < 256; ++idx) edge0[idx] = adsp_from_q31(edge0[idx]);
			if(!--read_count) break;
			else continue;
		}
		do_control: {
	...
	wola_rect_process(
		...
		stage_6_output,
		modules[6]->state);
	printf("input %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld\n", edge0[0], edge0[1], edge0[2], edge0[3], edge0[4], edge0[5], edge0[6], edge0[7], edge0[8], edge0[9], edge0[10], edge0[11], edge0[12], edge0[13]);
	printf("outpt %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld\n", edge6[0], edge6[1], edge6[2], edge6[3], edge6[4], edge6[5], edge6[6], edge6[7], edge6[8], edge6[9], edge6[10], edge6[11], edge6[12], edge6[13]);
	printf("err %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld\n", edge6[0] - edge0[0], edge6[1] - edge0[1], edge6[2] - edge0[2], edge6[3] - edge0[3], edge6[4] - edge0[4], edge6[5] - edge0[5], edge6[6] - edge0[6], edge6[7] - edge0[7], edge6[8] - edge0[8], edge6[9] - edge0[9], edge6[10] - edge0[10], edge6[11] - edge0[11], edge6[12] - edge0[12], edge6[13] - edge0[13]);
