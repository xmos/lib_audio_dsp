...
def get_sig(len = 0.25):
  ...
  sig_fl = np.round(0.8*np.sin(2*np.pi*997*time)*2**23)/2**23
  ...
  sig_fl, _ = sf.read(gen_dir / str(name + ".wav"))
...
def get_c_wav(run = True, sim = True):
  ...
  if run:
    ...
    sig_fl, _ = sf.read(gen_dir / "sig_c.wav")
    return sig_fl
...
def run_py(filt, sig_fl):
  ...
  int2, _ = sf.read(gen_dir / "sig_py_int.wav")
  filt.reset_state()
...
if __name__ =="__main__":
  test_biquad_c("biquad_notch", 20, 2)
