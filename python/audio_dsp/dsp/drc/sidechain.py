...
class compressor_rms_sidechain_mono(rms_compressor_limiter_base):
    ...
    def process_xcore(self, input_sample: float, detect_sample: float):  # type: ignore : overloading base class
        ...
        """
        # quantize
        sample_int = utils.float_to_fixed_signal(input_sample, self.Q_sig)
        detect_sample_int = utils.float_to_fixed_signal(detect_sample, self.Q_sig)
...
class compressor_rms_sidechain_stereo(rms_compressor_limiter_stereo_base):
    ...
    def process_channels_xcore(  # type: ignore : override base class
        ...
        for i in range(len(input_samples)):
            samples_int[i] = utils.float_to_fixed_signal(input_samples[i], self.Q_sig)
            detect_samples_int[i] = utils.float_to_fixed_signal(detect_samples[i], self.Q_sig)
        ...
        for i in range(len(input_samples)):
            ...
            y[i] = utils.fixed_to_float_signal(y_uq, self.Q_sig)
