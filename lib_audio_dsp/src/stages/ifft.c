...
void ifft_process(complex_spectrum_t ** input, int32_t ** output, void * app_data_state)
{
    ...
    bfp_complex_s32_init(&in_spect, input[0]->data, input[0]->exp, (state->nfft >> 1) + 1, 1);
    ...
    // if input and output pointer are different, copy- note input is nfft/2 + 1 complex, so can't be copied before
    if((int32_t *)input[0]->data != &output[0][0]) {
        memcpy(&output[0][0], input[0]->data, state->nfft*sizeof(int32_t));
    }
