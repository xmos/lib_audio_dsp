...
#include "stages/hard_limiter_peak.h"

static inline void limiter_copy_config_to_state(limiter_t *lim_state, int n_inputs, const hard_limiter_peak_config_t *lim_config)
{
...
static inline void limiter_copy_state_to_config(hard_limiter_peak_config_t *lim_config, const limiter_t *lim_state)
{
...
void hard_limiter_peak_process(int32_t **input, int32_t **output, void *app_data_state)
{
    ...
    hard_limiter_peak_state_t *state = app_data_state;
    ...
    do {
        ...
        int j = 0;
        do {
            *out++ = adsp_hard_limiter_peak(&state->lim[i], *in++);
...
void hard_limiter_peak_init(module_instance_t* instance, adsp_bump_allocator_t* allocator, uint8_t id, int n_inputs, int n_outputs, int frame_size)
{
    ...
    hard_limiter_peak_state_t *state = instance->state;
    hard_limiter_peak_config_t *config = instance->control.config;

    memset(state, 0, sizeof(hard_limiter_peak_state_t));
...
void hard_limiter_peak_control(void *module_state, module_control_t *control)
{
    xassert(module_state != NULL);
    hard_limiter_peak_state_t *state = module_state;
    xassert(control != NULL);
    hard_limiter_peak_config_t *config = control->config;
