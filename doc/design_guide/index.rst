...
For more information about the xcore architecture, consult the `The XMOS XS3 Architecture`_ manual.
...
The generated DSP threads and also the APIs for exchanging inputs with the pipeline all use channels to communicate audio. 
...
Thread usage of the DSP pipeline is discussed in the sections above. Understanding the thread usage of your application is a manual process. The application designer must have an understanding of how many threads are in use in there application as well as in the DSP pipeline to ensure that the limit of 8 is not exceeded. If this limit is exceeded the xcore will trap when the application attempts to fork a ninth thread.

Memory Usage
************

All memory used in the generated DSP pipeline is statically allocated and therefore known at compile time. The python design API will not assist in understanding the memory usage of your system. The memory report which is displayed when compiling the application must be consulted to see the memory used. This value will include the generated DSP pipeline as well as any other application code that is running on the tile.

MIPS Usage
**********

In order to operated in a real time audio system it is critical that each thread in the DSP pipeline can complete execution in less time that the sample period (or frame period if the frame size is greater than 1). It is this constraint that requires the DSP to be split into pipelined threads. The symptoms of an overloaded thread be that the output from the DSP pipeline is being produced at a lower than expected rate. This could cause the source and sink threads to block and miss timing. The current version of lib_audio_dsp provides only limited support for measuring the MIPS usage of each thread.

Each thread measures the total number of system ticks that pass while it is doing work and stores that maximum value that has occured since boot. This measurement can be used to get an estimate of the threads MIPS utilisation. To access this value, the function `adsp_auto_print_thread_max_ticks()` ("auto" may be replaced with a custom pipeline identifier if specified) is generated along with the other generated pipeline functions. Calling this on the same tile as the pipeline will print this value. Under the hood this uses `printf` so the output will only be visible when connected to the device with `xrun` or `xgdb`.

By default the system clock runs at 100MHz, the available ticks on each thread depends on the frame size and sample rate of the data. For example if the sample rate is 48000 and frame size is 1 then the available ticks will be :math:`1 * 100e6/48000 = 2083 ticks`. Below is an example output from `adsp_auto_print_thread_max_ticks()` for a pipeline with 4 threads::
...
The number that is displayed is the worst case that has happened since boot. This is not necesarily the worst case as some stages have data dependant execution time. Therefore, it is recommended to play an audio signal through the pipeline with varying amplitude and frequencies before measuring the thread MIPS.
